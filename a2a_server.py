# travel_a2a_server.py - Now using modular architecture
# This file serves as the main entry point while leveraging the modularized components
...
async def start_travel_agents_for_meeting(req: TravelBookingRequest):
    ...
    travel_session: Optional[AgentSession] = None
    flight_session: Optional[AgentSession] = None
    hotel_session: Optional[AgentSession] = None
    email_session: Optional[AgentSession] = None
...
async def test():
    ...
    return {
        "message": "VideoSDK Travel A2A Server with Email is running (Modular Architecture)!", 
        "agents": ["travel", "flight", "hotel", "email"],
...
if __name__ == "__main__":
    print("🚀 Starting Travel A2A Server with Modular Architecture")
    print("📁 Using components from: config/, models/, agents/, utils/")
    uvicorn.run("a2a_server:app", host="0.0.0.0", port=PORT, reload=True, log_level="info")